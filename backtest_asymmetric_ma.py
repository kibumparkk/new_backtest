...
# 데이터 로드
print("=" * 80)
print("비대칭 이동평균선 전략 백테스트 (벤치마크 능가 목표)")
print("=" * 80)
...
# === 비대칭 전략 탐색 ===
print("\n" + "=" * 80)
print("2. 비대칭 매수/매도 전략 최적화")
print("전략 컨셉: 매수는 보수적으로 (긴 MA), 매도는 적극적으로 (짧은 MA)")
print("또는 반대: 매수는 공격적으로, 매도는 느슨하게")
print("=" * 80)
...
# 전략 1: 매수는 보수적, 매도는 적극적
# 매수 MA: 20, 25, 30, 35, 40, 50 (보수적)
# 매도 MA: 5, 10, 15, 20, 25 (적극적)

buy_mas_conservative = [20, 25, 30, 35, 40, 50]
sell_mas_aggressive = [5, 10, 15, 20, 25]

# 전략 2: 매수는 공격적, 매도는 느슨하게
buy_mas_aggressive = [5, 10, 15, 20]
sell_mas_conservative = [40, 50, 60, 70]

# 모든 조합 테스트
all_combinations = []

# 보수적 매수 + 적극적 매도
for buy_ma in buy_mas_conservative:
    for sell_ma in sell_mas_aggressive:
        if buy_ma > sell_ma:  # 매수 MA가 매도 MA보다 길어야 함
            all_combinations.append(('conservative_buy', buy_ma, sell_ma))

# 공격적 매수 + 보수적 매도
for buy_ma in buy_mas_aggressive:
    for sell_ma in sell_mas_conservative:
        if buy_ma < sell_ma:  # 매수 MA가 매도 MA보다 짧아야 함
            all_combinations.append(('aggressive_buy', buy_ma, sell_ma))

print(f"\n총 {len(all_combinations)}개 전략 조합 테스트")
print("-" * 80)
print(f"{'전략타입':<20} {'매수MA':<8} {'매도MA':<8} {'Total Return':<15} {'CAGR':<10} {'MDD':<10}")
print("-" * 80)

all_results = []

for strategy_type, buy_ma, sell_ma in all_combinations:
    # 이동평균 계산
    ...
    strategy_label = "보수매수-빠른매도" if strategy_type == 'conservative_buy' else "빠른매수-보수매도"
    print(f"{strategy_label:<20} {buy_ma:<8} {sell_ma:<8} {total_return:<15.2f}x {cagr:<10.2%} {mdd:<10.2%}")

    # 최고 성과 추적
    if total_return > best_return:
        best_return = total_return
        best_params = (strategy_type, buy_ma, sell_ma)
        best_results = result
...
print(f"\n최적 전략 발견!")
strategy_label = "보수적 매수 + 빠른 매도" if best_params[0] == 'conservative_buy' else "빠른 매수 + 보수적 매도"
print(f"  전략 타입: {strategy_label}")
print(f"  매수 MA: {best_params[1]}")
print(f"  매도 MA: {best_params[2]}")
...
ax1.plot(df.index, df['strategy_equity'], label=f'Strategy (Buy MA={best_params[1]}, Sell MA={best_params[2]})', linewidth=2.5, color='#2E86AB')
...
ax1.set_title(f'Asymmetric MA Strategy: {strategy_label}', fontsize=16, fontweight='bold', pad=20)
ax1.legend(loc='upper left', fontsize=11)
ax1.grid(True, alpha=0.3, linestyle='--')

# 성과지표 텍스트 박스
status = "OUTPERFORM" if best_return > bench_total_return else "UNDERPERFORM"
...
performance_summary = pd.DataFrame({
    'Strategy': [f'{strategy_label}', 'Benchmark (SMA30)'],
    'Buy MA': [best_params[1], 30],
    'Sell MA': [best_params[2], 30],
...
# All tested strategies - 상위 20개만 저장
all_strategies_df = pd.DataFrame([
    {
        'Strategy Type': '보수매수-빠른매도' if r['strategy_type'] == 'conservative_buy' else '빠른매수-보수매도',
        'Buy MA': r['buy_ma'],
        'Sell MA': r['sell_ma'],
